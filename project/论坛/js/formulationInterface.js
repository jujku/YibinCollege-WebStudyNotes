...
axios({
  ...
  document.querySelector("#userdata").innerHTML =
    "发布人： " + response.data.data.author;
  document.querySelector("#usertime").innerHTML = response.data.data.creat_time;
  document.querySelector(".publishimgbtn").style.backgroundImage =
  ...
  document.querySelector("#base_liquor").innerHTML =
    response.data.data.base_liquor;
  const formulation = JSON.parse(response.data.data.formulation);
  let arr = [];
  for (k in formulation) {
    arr.push(
      '<div class="inin"><input readonly type = "text" id = "formualation_name" placeholder = "配料名" value = ' +
        k +
...
const getcomments = function () {
  axios({
    ...
    for (k in comments) {
      console.log(k);
      arr.push(
        '<div class="comment"><div class="commentuser"><p id="commentusername">' +
          comments[k].username +
          '</p><p id = "commenttime" > ' +
          comments[k].time +
